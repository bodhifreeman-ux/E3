...
echo '```' >> "$SUMMARY_FILE"
find /home/bodhifreeman/E3/E3 -type f -mmin -60 -not -path '*/\.*' -not -path '*/llama.cpp/*' 2>/dev/null | head -20 >> "$SUMMARY_FILE" || echo "None" >> "$SUMMARY_FILE"
echo '```' >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"

# Git status if in a repo
if git -C /home/bodhifreeman/E3/E3 rev-parse --git-dir > /dev/null 2>&1; then
    echo "### Git Status" >> "$SUMMARY_FILE"
    echo '```' >> "$SUMMARY_FILE"
    git -C /home/bodhifreeman/E3/E3 status --short >> "$SUMMARY_FILE" 2>/dev/null || echo "No changes" >> "$SUMMARY_FILE"
    echo '```' >> "$SUMMARY_FILE"
    echo "" >> "$SUMMARY_FILE"

    echo "### Recent Commits" >> "$SUMMARY_FILE"
    echo '```' >> "$SUMMARY_FILE"
    git -C /home/bodhifreeman/E3/E3 log --oneline -5 >> "$SUMMARY_FILE" 2>/dev/null || echo "No commits" >> "$SUMMARY_FILE"
    echo '```' >> "$SUMMARY_FILE"
    echo "" >> "$SUMMARY_FILE"
fi
